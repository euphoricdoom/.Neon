...
def resolve_origin_state(artifact_path: str | Path):
    ...
    if expected_hash and expected_hash != current_hash:
        ...
        return OriginState.CONFLICTED_ORIGIN, signals, 0.0, ["inspect_hash_mismatch"], None
...
def default_execution_decision(origin_state: OriginState, action: str = "run") -> ExecutionDecision:
    ...
    if origin_state == OriginState.UNKNOWN_ORIGIN:
        return ExecutionDecision.ALLOW_SANDBOXED
    if origin_state == OriginState.CONFLICTED_ORIGIN:
        return ExecutionDecision.ALLOW_READ_ONLY
    return ExecutionDecision.ALLOW_LIMITED


def resolve_execution_decision(artifact_path: str | Path, *, action: str = "run", override_mode: str | None = None, override_reason: str | None = None) -> OriginGateResult:
    ...
    if override_mode == "creator-local" and origin_state == OriginState.CONFLICTED_ORIGIN:
